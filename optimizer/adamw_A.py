...
class AdamW_A(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdamW_A, self).__init__(params, defaults)
        self.eps = eps

    def __setstate__(self, state):
        super(AdamW_A, self).__setstate__(state)
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=None, grad_norms=None, grad_scaler=None):
        ...
        for group in self.param_groups:
            for p in filter(lambda p: exists(p.grad), group['params']):
                ...
                real_update_tmp = -step_size * torch.mul(p.data, group['weight_decay']).addcdiv_(exp_avg, denom,
                                                                                                 value=1.0)
                #real_update_wo_lr_tmp = torch.mul(p.data, group['weight_decay']).addcdiv_(1, exp_avg, denom)

                #real_update_tmp = -step_size * torch.mul(p.data, group['weight_decay']).addcdiv_(exp_avg, denom,value=1.0)
                #real_update_wo_lr_tmp = torch.mul(p.data, group['weight_decay']).addcdiv_(exp_avg, denom, value=1.0)
